...
- [기술 스택](#기술-스택)
- [로컬 실행](#로컬-실행)
- [로컬 부하 테스트 (k6 · Prometheus · Grafana)](finready-backend/load-test/README.md)
- [환경 변수](#환경-변수)
- [테스트](#테스트)
- [배포](#배포)
...
│   ├── openapi.yml                          API 계약 원본 (v1.4.6) ← 단일 원천
...
타임아웃·재시도·폴백·검증·비용은 [AI 연동 설계](#ai-연동-설계)에 모아 뒀다.
...
LLM은 **판정을 제안할 뿐 결정하지 않는다.** 아래는 그 전제를 코드로 지키기 위해 정한 것들이다.

### 호출 지점

Claude 호출은 `ai/AiGateway` 한 곳을 통과한다. 프롬프트를 만드는 것은 각 포트 구현체가 하고,
게이트웨이는 **호출·캐싱·재시도·기록**만 한다. 모아둔 이유가 셋이다.

1. prompt caching을 안 쓰면 크레딧이 금방 마른다 — 시스템 프롬프트가 전 세션 공통이라 캐시 prefix로 딱 맞는다
2. 재시도 정책이 흩어지면 "1회만"(TRD §7.1)이 지켜지는지 확인할 수 없다
3. `llm_call_log`는 평가 재현의 원천이라(TRD §7.2) 빠뜨리는 경로가 있으면 안 된다

**API 키가 없어도 서버는 뜬다.** `AiPortConfig`가 스텁을 등록해 F01·F02처럼 LLM이 필요 없는
경로를 막지 않고, 기동 로그에 어느 모드인지 한 줄 남긴 뒤 호출되는 순간 명확히 터진다.

### 실패를 다루는 방식

**타임아웃 60초.** 원래 30초였는데 실측에서 classifier 단독 23.3초가 나왔다(`CONS_A_002`).
여유가 6.7초인데 그 상담문은 1,400자이고 **계약 상한은 8,000자**다 — 레이턴시는 출력 토큰에
비례하고 출력은 Risk 9개 × 인용문이라 상담문이 길수록 늘어난다. 근본 해결(배치 병렬화)은
별건이었으므로, 그때까지 **실패 모양을 "느림"으로 두기 위해** 올린 값이다.

**재시도는 두 층이다.** 성격이 달라서 합치지 않았다.

| 층 | 담당 | 값 |
|---|---|---|
| SDK (`AnthropicOkHttpClient.maxRetries`) | 전송 계층 — 429 · 5xx · 연결 끊김 | 1 |
| `AiGateway.call()` 루프 | **파싱 실패** | 최대 2회 시도 |

파싱에 실패했을 때 호출부가 `call()`을 다시 부르는 게 아니라, `parser`를 게이트웨이에 넘겨
여기서 재시도하게 했다. 그래야 "1회만"이 한 곳에서 지켜진다. 최악은 `60초 × 2 = 120초`다.

**폴백은 포트마다 다르다.** 한 줄로 통일했으면 사고가 났다.

| 포트 | 실패하면 | 왜 |
|---|---|---|
| `QuestionGenerator` · `ReExplanationGenerator` | 검수된 fallback으로 대체, `source: FALLBACK` | 계약이 정한 **정상 경로**다. 검수된 문장이라 내용이 틀릴 수 없다 |
| `CoverageClassifier` · `SemanticVerifier` · `AnswerJudge` | 던진다 (503) | 빈 결과를 돌려주면 **전 Risk가 "설명 안 됨"으로 읽혀 Gate가 잠긴다** |

LLM 실패는 `AI_TIMEOUT` / `AI_PARSING_FAILED` → **503 + `recoverable=true`**다. 502/504가
아닌 건 계약이 "LLM 1회 재시도 후에도 실패"를 그렇게 정의해서고, 두 코드를 나눈 건 프론트가
"잠시 후 다시"와 "응답을 이해하지 못했습니다"를 다르게 안내할 수 있게 하기 위해서다.

**같은 요청을 반복해도 LLM을 다시 부르지 않는다** — 새로고침이 요금과 레이턴시를 다시 물게
하지 않는다. 멱등 지점은 [핵심 도메인 규칙 §7](#7-멱등성) 참조.

### 응답 구조화와 검증

**Sonnet 4.6은 structured outputs를 지원하지 않는다.** 스키마를 API에 위임할 수 없으므로
프롬프트로 JSON을 유도하고 `ai/JsonResponses`가 방어적으로 파싱한다 — **규칙 9("enum 밖 값은
파싱 실패, 임의 매핑 금지")의 보장이 API가 아니라 우리 코드에 있다는 뜻이다.**

- 모델이 코드펜스나 앞뒤 설명을 붙이는 일이 잦아 첫 `{`부터 마지막 `}`까지만 잘라 쓴다
- 필수 필드가 없거나 비면 파싱 실패다. **빈 값을 기본값으로 채우지 않는다**
- `requireEnum`은 목록 밖 문자열을 **비슷한 값으로 매핑하지 않는다.** 그렇게 하면 모델이
  만들어낸 값이 판정으로 둔갑한다

응답을 받은 뒤의 검증은 4겹이다 — provenance(원문 재검색) × semantic(2차 호출) × 결정표 ×
DB check 제약. 상세와 결정표는 [핵심 도메인 규칙 §3](#3-coveragestatus는-provenance--semantic의-결과다)에 있다.
**분류기는 evidence의 offset을 아예 받지 않는다** — 모델에게 위치를 묻지 않으면 틀린 위치를
받을 일도 없고, 서버가 원문에서 다시 찾는다(규칙 4).

재설명에만 **Guardrail**이 하나 더 붙는다(`explanation/Guardrail`). 고객에게 그대로 읽히는
문장이라 위험 축소·가입 권유·근거 없는 숫자를 막는데, **LLM 자가검증이 아니라 금칙어 방식**이다 —
결정적이라 테스트로 고정되고 실행마다 흔들리지 않는다. Coverage 실측에서 경계 판정이 실행마다
뒤집히는 걸 봤으므로 그 성질을 안전장치에 넣지 않았다.

> **이 클래스의 핵심은 부정형 예외다.** "보장"·"확정"·"안전"을 단순 `contains`로 막으면
> **맞는 설명이 걸린다** — 이 상품의 검수된 사실 자체가 부정형이다("원금이 보장되지 않습니다").
> 그대로 구현했다면 가장 정확한 재설명이 매번 fallback으로 떨어졌을 것이다. 그래서 이 단어들은
> 뒤따르는 **같은 절 안에** 부정 표현이 없을 때만 위반으로 센다. 절 경계를 끊지 않으면
> "원금이 보장됩니다. 손실은 없습니다"의 "없"이 앞 문장의 "보장"을 면제해 버린다.
> 목록을 늘릴 때는 실제 생성물에서 근거를 확보한다 — 짐작으로 넣으면 이번엔 위반을 놓치는 쪽으로 틀린다.

**프롬프트와 판정 로직은 분리돼 있다.** 프롬프트는 각 `Claude*` 구현체의 `SYSTEM_PROMPT`
상수에, 판정은 `CoverageStatusResolver`(결정표) · `GateEvaluator`(Gate 판정식) ·
`NextActionResolver`(분기)에 있다. **Gate 판정에 LLM이 개입하지 않는다** — 모델은 4상태
분류만 하고, 그것이 문을 여는지는 TRD §8.6 식을 그대로 옮긴 코드가 정한다. 프롬프트 버전은
상수로 박혀 매 호출마다 `llm_call_log.prompt_version`에 남는다(Hold-out 재현 조건).

### 민감정보

고객 프로파일 6건은 전부 합성이고(`synthetic: true`), 스키마에 이름·연락처·계좌번호 같은
식별자가 **아예 없다.** 그 위에 로그를 한 번 더 좁혔다.

`llm_call_log.request_summary`는 프롬프트 전문이 아니라 **메타데이터만** 담는다.

```
riskId=R01, attempt=1, answerChars=42                  ← 답변 본문이 아니라 길이
risks=3/9, batch=[R01,R02,R03], transcriptChars=1400   ← 상담 원문이 아니라 길이
```

`audit_event`도 상담 원문·고객 답변·미해결 사유 본문을 넣지 않는다 — 원본 테이블에 이미 있고,
append-only라 한 번 들어가면 지울 수 없다. API 키는 `AiProperties.toString()`을 오버라이드해
마스킹한다. record 기본 `toString`은 모든 필드를 찍는데 **Spring이 바인딩 실패 시 대상 객체를
오류 메시지에 넣으므로 실제로 새는 경로다.**

> ⚠️ **다만 `llm_call_log.raw_response`는 남는다**(8,000자 초과 시 절단). 여기엔 모델이 인용한
> 상담 원문 구절이 그대로 들어간다. 지금은 상담문도 합성이라 문제가 없지만, 실제 상담에 쓴다면
> **이 컬럼이 상담 내용의 부분 사본이 된다.** 응답 원문 없이는 판정을 재현할 수 없어 남기고 있는
> 것이며, 보존 기간과 마스킹은 그때 다시 정해야 한다.

### 비용과 레이턴시

**비용** — 실측 세션당 약 **$0.11**(콜드, Risk 2건 처리 기준) → $5 크레딧으로 약 45세션.

- **prompt caching** — 시스템 프롬프트를 캐시 prefix로 쓴다. 캐시는 prefix 일치라 이 앞에
  변하는 값을 두면 전부 무효가 된다. 실측 `cacheWrite=2969 → cacheRead=2969` 완전 적중, 44% 절감
- **TTL 1시간**(기본 5분) — 심사가 09-07~09-11 닷새에 걸쳐 띄엄띄엄 들어오면 5분으로는
  세션 간 캐시가 거의 안 겹쳐 **쓰기만 물고 읽기 혜택이 없다**
- **effort를 단계마다 다르게 준다.** 안 걸면 기본이 `high`라 분류·다듬기에는 과하다

| 포트 | effort | 이유 |
|---|---|---|
| `SemanticVerifier` | `HIGH` | 근거가 사실을 지지하는지 판단하는 자리다. 여기서 아끼면 정확히 그 단계의 재현율이 떨어진다 |
| `CoverageClassifier` · `AnswerJudge` | `MEDIUM` | 구조화 추출에 가깝지만 `CONTRADICTED` 판별에 의미 추론이 필요해 `LOW`로는 안 내린다 |
| `QuestionGenerator` · `ReExplanationGenerator` | `LOW` | 판정이 아니라 문장 다듬기다. 넣을 내용이 이미 주어져 추론할 것이 없다 |

**레이턴시** — classifier가 9개 Risk를 **3개씩 3배치로 나눠 가상 스레드로 동시 호출**한다.
회귀 실측에서 이 단계의 레이턴시가 **출력 토큰에 비례**하고 고정 오버헤드는 4,562ms뿐임이
확인돼(n=31), 출력을 3등분하면 세 배치가 병렬로 도는 동안 각자의 출력이 1/3이 된다.
결과는 **41~51% 단축, 정확도 손실 0**(Risk 120/135 · Gate 15/15로 팬아웃 전과 동일).

- **Risk마다 부르지는 않는다.** 9회로 쪼개면 고정 오버헤드 4.5초를 9번 물고 캐시 쓰기도 9번이다.
  분할의 이득은 출력 토큰에서 나오지 호출 수에서 나오지 않는다
- **배치 크기 3은 커넥션 풀에서 역산했다.** 각 배치가 `LlmCallRecorder`를 부르므로 한 요청이
  순간 커넥션을 배치 수만큼 쓴다. Hikari 풀이 5(DB role 한도와 맞춤)라 3을 넘기면 경합한다
- **Risk 카탈로그는 배치마다 자르지 않고 전부 system에 둔다.** 배치마다 다른 카탈로그를 넣으면
  캐시 prefix가 배치 수만큼 갈라져 전부 miss가 되는데, **그 사고는 오류도 로그도 테스트 실패도
  없이 청구서로만 나타난다**

**관측** — `llm_call_log`에 stage · promptVersion · model · latencyMs · attempt · parsedOk와
토큰 4종(input / output / cacheRead / cacheWrite)이 남는다. `REQUIRES_NEW` 트랜잭션이라
**호출부가 나중에 실패해도 기록은 남는다** — 파싱 실패로 롤백되면 정작 원인인 응답 원문이
사라져 재현할 수 없다.

> **캐시가 붙었는지는 `cacheRead`로만 알 수 있다.** 캐시 미달(모델별 최소 토큰)이면 **오류 없이
> 조용히** 안 걸린다. 실제로 F04~F07 세 단계는 시스템 프롬프트가 Sonnet 4.6의 1024토큰 최소치
> 미만이라 캐시가 전혀 안 붙는데, `cacheWrite=0`이 유일한 신호였다. **고치지 않았다** — 캐시를
> 태우려고 프롬프트를 늘리면 호출당 $0.003 아끼자고 매번 입력을 늘리는 셈이다. 레이턴시로도
> 판단할 수 없다. 실행 간 편차가 캐시 효과보다 크다는 것이 이미 관측됐다
> (같은 프롬프트로 verifier 5.0s ~ 12.0s).
...
`docs/openapi.yml`(현재 **v1.4.6**)이 단일 원천이고, **백엔드만 수정한다.**
...
- **Guardrail은 단순 금칙어로는 안 된다.** 이 상품의 검수된 사실 자체가 부정형이라
  맞는 설명이 걸린다 — 경위와 대응은 [AI 연동 설계 > 응답 구조화와 검증](#응답-구조화와-검증) 참조.
