...
마감(2026-09-06)까지 시간이 넉넉하지 않고 DoD가 12초를 요구하지 않으므로, 이 결과를
...
**해소 (2026-08-26, 팀 결정).** 사본을 두지 않는 쪽으로 결정 — 프론트가 루트
`docs/openapi.yml`을 직접 참조한다. `package.json`의 `gen:api`·`contract.test.ts` 경로
수정은 프론트 쪽 작업. 상세는 `finready-backend/CLAUDE.md`.
